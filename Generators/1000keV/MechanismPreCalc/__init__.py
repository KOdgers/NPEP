...
#                         print(l1, '.', l2, '.', l3)
    ...
    def MultipleSweeps(self, iter_number=0, n = 10, expcounts=[5,10],Res = [0,50],Input_HL='Halflives48.npy',tol=.1):
        ...
        config_number = int(np.floor(iter_number/1000))
...
#             print('Def Tau: ',taus[ind])
            for ik in range(0,len(KTContenders)):
                if ik ==0:
                    minKTtemp , minKTloctemp = self.GetBestKT1000(samples,KTContenders[ik],res=Res,tol=Tol,oldarr=precalcarr)
                else:
                    minKTtemp , minKTloctemp = self.GetBestKT1000(samples,KTContenders[ik],res=Res,tol=Tol)
