...
#                         print(l1, '.', l2, '.', l3)
    ...
    def MultipleSweeps(self, iter_number=0, n = 10, expcounts=[5,10],Res = [0,50],Input_HL='Halflives48.npy',tol=.1):
        ...
        config_number = int(np.floor(iter_number/3000))
...
#            tempT = tempT
        ...
        for samplenum in range(0,len(samples)):
            ...
            x,t = sample[0],sample[1]
            if Res!=1:
                tempK = np.load('KE_Linear_'+str(int(Res*1000))+'_'+str(int(x/100))+'_setsV3.npy')[ind[0],ind[1],ind[2],int((x/(Res*1000))%modx)]
            else:
...
#             print('Def Tau: ',taus[ind])
            for ik in range(0,len(KTContenders)):
                if ik ==0:
                    minKTtemp , minKTloctemp = self.GetBestKT10(samples,KTContenders[ik],res=Res,tol=Tol,oldarr=precalcarr)
                else:
                    minKTtemp , minKTloctemp = self.GetBestKT10(samples,KTContenders[ik],res=Res,tol=Tol)
...
#             print('======================================================== \n \n')
    ...
    def GetBestKT10(self,data,loc,res,var=10,tol=.01,oldarr=False):
                        ...
                        for x in range(0,300):
                            if Res*1000 == 10:
                                if x ==0 :
                                    ...
                                    x0 = self.Qbb - Res
                                    xarr = np.linspace(2.98, self.Qbb-.0001, 50)
                                else:
#                                     x0 = (x/1000)-Res/2
                                    xarr = np.linspace(x/100, (x/100)+Res, 50)
                                y = []
                                for xiter in xarr:
                                    y.append( self.KineticDist(xiter/me))
                                tempK.append(simps(y,xarr))
                        KTPre[l1,l2,l3,:]=np.asarray(tempK)/np.sum(tempK[0:299])
